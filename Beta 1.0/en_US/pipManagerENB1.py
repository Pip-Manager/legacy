...
class MainForm(QWidget):
    def __init__(self):
        ...
        self.headers = ['ID', 'Name', 'Version']
        self.setWindowTitle("PIP Manager Beta 1.0.0")
        self.setFixedSize(710, 800)
        self.pipFormStatus = 0
        self.pipFormStatusFind = 0

        self.pipList = QTableWidget(self)
        self.controlLabel = QLabel("Control Center:", self)
        self.addButton = QPushButton("Install", self)
    ...
    def startAddTmp(self):
        if self.newLine.text():
            ...
            if "No matching distribution found for " in stderr:
                QMessageBox.critical(self, "Error", "Can not find lib: " + downloadPip, QMessageBox.Yes)
            else:
                QMessageBox.information(self, "Successes", downloadPip + " Install successfully", QMessageBox.Yes)
    ...
    def addPip(self):
        ...
        self.newName = QLabel("Enter PIP lib name(Only one):", self)
        self.newLine = QLineEdit(self)
        self.startAddButton = QPushButton("Install", self)
        self.closeAddButton = QPushButton("Close", self)
    ...
    def startDelTmp(self):
        if self.delLine.text():
            ...
            if "as it is not installed." in stderr:
                QMessageBox.critical(self, "Error", "Can not find lib: " + deletePip, QMessageBox.Yes)
            else:
                QMessageBox.information(self, "Successes", deletePip + " Uninstall successfully", QMessageBox.Yes)
    ...
    def delPip(self):
        if self.pipFormStatus:
            return
        self.pipFormStatus = 1
        self.delName = QLabel("Enter PIP lib name(Only one):", self)
        self.delLine = QLineEdit(self)
        self.startDelButton = QPushButton("Uninstall", self)
        self.closeDelButton = QPushButton("Close", self)
    ...
    def startFindTmp(self):
        ...
        if self.findLine.text():
            ...
            if "Package(s) not found:" in stderr:
                QMessageBox.critical(self, "Error", "Can not find lib: " + findPip, QMessageBox.Yes)
            else:
                self.pipInfoLabel = QLabel("Pip Lib Information:", self)
    ...
    def findPip(self):
        if self.pipFormStatus:
            return
        self.pipFormStatus = 1
        self.findName = QLabel("Enter PIP lib name(Only one):", self)
        self.findLine = QLineEdit(self)
        self.startFindButton = QPushButton("Search", self)
        self.closeFindButton = QPushButton("Close", self)
...
class AboutPage(QDialog):
    def __init__(self):
        ...
        self.setWindowTitle("PIP Manager About")
        self.resize(550, 200)
        self.authorLabel = QLabel("Author:", self)
        self.qqLabel = QLabel("QQ:", self)
        self.mailLabel = QLabel("Mail:", self)
        self.githubLabel = QLabel("Github:", self)
        self.giteeLabel = QLabel("Gitee:", self)
        ...
        self.githubButton = QPushButton("Visit Github Repo", self)
        self.giteeButton = QPushButton("Visit Gitee Repo", self)
